...
namespace Lean
{
	...
	public class LeanPool : MonoBehaviour
	{
		...
		public static List<Lean.LeanPool> AllPools = new List<Lean.LeanPool>();
		
		// The reference between a spawned GameObject and its pool
		public static Dictionary<GameObject, Lean.LeanPool> AllLinks = new Dictionary<GameObject, Lean.LeanPool>();
		...
		public static GameObject Spawn(GameObject prefab, Vector3 position, Quaternion rotation, Transform parent)
		{
			if (prefab != null)
			{
				...
				if (pool == null)
				{
					pool = new GameObject(prefab.name + " Pool").AddComponent<Lean.LeanPool>();
		...
		public static void Despawn(GameObject clone, float delay = 0.0f)
		{
			if (clone != null)
			{
				var pool = default(Lean.LeanPool);
				...
				{
					//Debug.LogError("Attempting to despawn " + clone.name + ", but failed to find pool for it! Make sure you created it using Lean.LeanPool.Spawn!");
